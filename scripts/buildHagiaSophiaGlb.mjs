...
// 绕 x 轴倾斜的薄板（做侧廊单坡屋面：沿 x 铺满、沿 y 倾斜）
function addBoxRotX(key, sx, sy, sz, x, y, z, rx) {
  const g = new THREE.BoxGeometry(sx, sy, sz);
  g.rotateX(rx);
  g.translate(x, y, z);
  pushGeom(g, key);
...
for (const y of [-0.42, 0.42]) {
  ...
  addBoxRotX('wallShade', 1.08, 0.22, 0.032, 0, 0.445 * face, 0.545, -0.279 * face);
  // 屋脊压条（沿中殿一侧的高边收口，避免与中殿墙穿插出现破缝）
  addBox('cornice', 1.10, 0.024, 0.030, 0, 0.337 * face, 0.578);
  // 侧廊一排拱窗暗块
  for (let i = -4; i <= 4; i += 1) {
    addBox('arch', 0.045, 0.04, 0.16, i * 0.105, y - face * 0.115, 0.30);
  }
}
// 中殿一排高拱窗（南北面）
...
for (const [x, y] of [[-0.36, -0.36], [0.36, -0.36], [-0.36, 0.36], [0.36, 0.36]]) {
  ...
  addConeUp('cornice', 0.106, 0.075, x, y, 0.62, 4); // 墩顶小帽（角与墩顶对齐，不外挑）
}
// 外侧扶壁飞券暗示（南北各两道斜墩）
...
for (const [x, y] of [[-0.62, -0.48], [0.62, -0.48], [-0.62, 0.48], [0.62, 0.48]]) {
  // 基座 kürsü（略粗的多边形墩，贴地）
  addCyl('minaret', 0.080, 0.092, 0.16, x, y, 0.08, 12);   // z 0.00..0.16
  addCyl('minaretCap', 0.064, 0.082, 0.045, x, y, 0.185, 12); // 收分过渡环 z 0.16..0.21
  // 主塔身：单段连续微收分（16 边接近圆，不再像积木堆叠）
  addCyl('minaret', 0.040, 0.062, 0.70, x, y, 0.56, 16);   // z 0.21..0.91
  // 阳台 şerefe：下托外撑 + 金环 + 栏板
  addCyl('minaretCap', 0.052, 0.044, 0.05, x, y, 0.915, 16); // 出挑托 z 0.89..0.94
  addCyl('gold', 0.066, 0.066, 0.030, x, y, 0.955, 16);    // 阳台金环 z 0.94..0.97
  addCyl('minaret', 0.046, 0.044, 0.024, x, y, 0.982, 16); // 栏板 z 0.97..0.994
  // 上段塔身（继续收分）
  addCyl('minaret', 0.030, 0.042, 0.16, x, y, 1.07, 16);   // z 0.99..1.15
  // 锥形尖顶 külah（细长铅笔尖）
  addConeUp('minaretCap', 0.040, 0.30, x, y, 1.13, 16);    // z 1.13..1.43
  addSphere('gold', 0.020, x, y, 1.44, 10);                // 金顶尖
}
