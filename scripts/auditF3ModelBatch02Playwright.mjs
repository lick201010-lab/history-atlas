...
const appUrl = process.env.F3_BATCH02_URL || 'http://127.0.0.1:4188/';
const chromePath = process.env.CHROME_PATH || 'C:\\Program Files (x86)\\Microsoft\\Edge\\Application\\msedge.exe';
const debugPort = Number(process.env.F3_BATCH02_CDP_PORT || 9232);
const outDir = path.join(process.cwd(), 'docs', 'model-qa');
const userDataDir = path.join(process.cwd(), '.claude-runs', 'cdp-f3-batch02');
const manifestPath = path.join(outDir, 'f3-batch02-manifest.json');
const reportPath = path.join(outDir, 'F3_BATCH02_REPORT_2026-06-06.md');

const scenes = [
  {
    id: 'ziggurat-ur-1800bce',
    targetId: 'ziggurat-ur',
    title: '乌尔金字形神塔',
    year: -1800,
    camera: { center: [46.1, 30.96], zoom: 7.2, pitch: 62, bearing: 34 },
    screenshot: 'f3-batch02-ziggurat-ur-1800bce.png',
  },
  {
    id: 'ishtar-gate-575bce',
    targetId: 'ishtar-gate',
    title: '伊什塔尔门',
    year: -575,
    camera: { center: [44.42, 32.54], zoom: 7.1, pitch: 64, bearing: -28 },
    screenshot: 'f3-batch02-ishtar-gate-575bce.png',
  },
];
...
try {
  ...
  const modelResponses = client.events
    .filter((event) => event.method === 'Network.responseReceived' && /\/models\/(ziggurat-ur|ishtar-gate)\.glb/.test(event.params.response.url))
  ...
  const report = [
    '# F3 Batch02 Model QA',
    '',
    'Scope: `ziggurat-ur` and `ishtar-gate` GLB coverage and selected map-view QA.',
    '',
    `App URL: ${appUrl}`,
    ...
    '## Visual Notes',
    '',
    '- `ziggurat-ur` adds a stepped mudbrick ziggurat with approach stairs/ramps, buttresses, brick bands, parapets, and summit shrine.',
    '- `ishtar-gate` adds a blue-glazed Babylonian gate with twin towers, central arch, battlements, relief accents, and side returns.',
    '- Additional in-app browser QA confirmed both cards stayed open, immersive mode activated, Chinese titles rendered correctly, and page logs were empty.',
    '- Visual grade: both are accepted as B-grade readable coverage miniatures. The full F3 Gate remains incomplete.',
  ].join('\n');
