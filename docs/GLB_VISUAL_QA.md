...
- The weakest assets are `chichen-itza`, `petra`, `great-wall`, and `forbidden-city`.
...
| `chichen-itza` | [09](glb-visual-qa/09-chichen-itza.png) | Fail | Very low triangle count confirmed by audit. In map view it is too small/simple and lacks stairs/temple detail. | Rebuild first. Add four stairways, top temple, bevels, and stepped shadow layers. |
...
1. Fix `chichen-itza`: this is the only model that is both audit-warned and visually weak.
2. Fix `petra`: current shape does not communicate the famous rock-cut facade.
3. Fix `great-wall`: current model is not wall-like enough for the landmark.
4. Fix `forbidden-city`: solve the blue side artifact and label/model overlap.
5. Add a landmark focus mode: when a user clicks a landmark, dim or simplify territory fills and suppress nearby non-selected labels/models for a cleaner inspection view.
6. Revisit coastline-aware boundaries after the model pass. The screenshots show that boundary polygons still cross water or form coarse diagonal cuts in several regions.
